...
var pkg       = JSON.parse(fs.readFileSync(__dirname+'/'+ pkg_file, 'utf8'));
...
function get_system_listen_addresses() {
  var networkInterfaces = os.networkInterfaces();
  var addresses = [];
  for(var key in networkInterfaces) {
    networkInterfaces[key].forEach(function(interfaceInstance){
      if (interfaceInstance.family === 'IPv4') {
        addresses.push(interfaceInstance.address + ':4873');
      }
    });
  }
  return addresses;
...
function get_listen_addresses() {
  ...
  if (commander.listen) {
    ...
    addresses = get_system_listen_addresses();
  }
